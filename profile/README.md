...
## Collaboration
1. PM / Integration: 전체 일정 관리, 기능 우선순위 정리, 구조 설계, 최종 통합 및 데모 준비를 담당합니다.
2. Frontend Editor / UI Owner: React 기반 에디터 UI, 편집 기능, 협업 화면, 문서 생성·열기 흐름과 저장 연동 경험을 담당합니다.
3. Backend Realtime / API Owner: Rust 서버, 실시간 동기화, 동시 편집 충돌 처리를 위한 구조, 저장 및 복구 구조를 담당합니다.
4. QA / Docs / DevOps Owner: `.docx` import 검증, 파일 처리 검증, 테스트 개발 계획, 문서 최신화와 운영 준비를 담당합니다.
